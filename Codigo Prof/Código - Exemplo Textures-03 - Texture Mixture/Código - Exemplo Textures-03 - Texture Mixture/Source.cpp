...
void load_texture(vector<string> textureFiles);
void init(void);
void display(void);
...
int main(void) {
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "OpenGL - Texture Mixture", NULL, NULL);
	if (window == NULL) {
		glfwTerminate();
		return -1;
	}
	...
	vector<string> textureFiles{ 
		"textures/StoneWall.gif",	// 256x256
		"textures/graffiti.png"		// 256x512 c/ transparência
	};
	load_texture(textureFiles);
	...
	GLint locationTextureSampler1 = glGetProgramResourceLocation(programa, GL_UNIFORM, "TexSampler1");
	glProgramUniform1i(programa, locationTextureSampler1, 0 /* Unidade de Textura #0 */);
	// Indicação da Unidade de Textura a ligar ao sampler 'TexSampler2'.
	GLint locationTextureSampler2 = glGetProgramResourceLocation(programa, GL_UNIFORM, "TexSampler2");
	glProgramUniform1i(programa, locationTextureSampler2, 1 /* Unidade de Textura #1 */);
...
void load_texture(vector<string> textureFiles) {
	...
	for (auto file : textureFiles) {
		...
		cout << file << "-> Width:" << width << " Height:" << height << " Channels:" << nChannels << endl;
		if (imageData) {
